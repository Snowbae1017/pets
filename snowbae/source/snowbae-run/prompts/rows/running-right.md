Create a single horizontal sprite strip for the Codex app digital pet `snowbae` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Create a respectful, non-photorealistic cartoon desktop pet based on the provided child references. Preserve the recognizable winter outfit and real-life demeanor: gentle, curious, quietly lively, with a soft confident expression. Identity anchors: fluffy cream winter hat, pale blue long robe, white faux-fur collar and cuffs, colorful bead details, dark braided hair, cozy tan winter boots, snow-princess feeling. Do not make an exact realistic portrait; simplify into a friendly chibi digital pet while keeping the natural posture and spirit..
